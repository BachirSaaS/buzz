...
pub async fn validate_standard_deletion_event(
    ...
    for target_id in target_ids {
        ...
        if u32::from(target_event.event.kind.as_u16()) == buzz_core::kind::KIND_WORKFLOW_DEF {
            return Err(anyhow::anyhow!(
                "workflow deletion requires a canonical a-tag coordinate"
            ));
        }
...
pub async fn validate_admin_event(
    ...
    match kind {
        ...
        9005 => {
            ...
            if u32::from(target_event.event.kind.as_u16()) == buzz_core::kind::KIND_WORKFLOW_DEF {
                return Err(anyhow::anyhow!(
                    "workflow deletion requires an author-signed canonical a-tag coordinate"
                ));
            }
...
async fn handle_a_tag_deletion(
    ...
    match kind_num {
        ...
        buzz_core::kind::KIND_WORKFLOW_DEF => {
            // All workflow deletion belongs to the atomic pre-storage lifecycle.
            // Do not retain a name/UUID runtime-only deletion escape hatch here.
            return Err(anyhow::anyhow!(
                "workflow deletion requires the atomic lifecycle path"
            ));
        }
        // Other NIP-33 kinds keep their existing timestamp-ordered soft deletion.
