...
pub(crate) mod tests {
    ...
    async fn f5_loopback_teardown_epilogue_removes_subscription_and_topic_refcount() {
        ...
        let state_arc = crate::state::tests::test_state_with_database_pool(pool).await;
        ...
        // A global-scoped REQ never retains a channel topic. Assert it for completeness.
        let channel_refcount = state
            .pubsub
            .topic_refcount(&tenant, EventTopic::Channel(Uuid::nil()))
            .await;
        assert_eq!(
            channel_refcount, 0,
            "F5: channel topic refcount must be zero (global-scoped REQ retains no channel topic)."
        );
