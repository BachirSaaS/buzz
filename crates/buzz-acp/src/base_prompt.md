...
- Your commit author identity is machine-managed: the runtime's Git config authors and signs every commit as your agent identity (email `<pubkey>@<relay-host>`). Never set `user.name`/`user.email`, and never pass `-c user.*`, `--author`, or `--reset-author`; the managed `git` refuses those when it is the `git` your shell resolves, but that is a safeguard, not permission to try. Credit the human operator with the `Co-authored-by` and `Signed-off-by` trailers the repo requires (add them to the commit message body); if you cannot determine the operator's email for those trailers, stop and ask. (When the operator sets `BUZZ_GIT_IDENTITY=user`, commits instead carry their own git identity and these trailers are redundant.)
