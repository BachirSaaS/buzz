...
impl DeletionStore {
    ...
    /// Terminally abort a reversible request before object deletion begins.
    pub async fn abort(
        &self,
        request_id: Uuid,
        aborted_by: &str,
...
mod postgres_tests {
    ...
    #[tokio::test]
    #[ignore = "requires Postgres"]
    async fn privileged_abort_recovers_inventoried_request_without_unarchiving() {
        let (db, store) = store().await;
        let (host, owner, community) = archived_owned_community(&db).await;
        let operator = "bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb";
        let request_id = Uuid::new_v4();
        ...
        store
            ...
            .await
            .expect("freeze inventory through manual seam");
        let aborted = store
            .abort(
                request.id,
                "recovery-operator",
                "inventory requires recovery",
            )
            .await
            .expect("abort inventoried request");
        assert_eq!(aborted.stage, DeletionStage::Aborted);
        let archived_at: Option<DateTime<Utc>> =
            sqlx::query_scalar("SELECT archived_at FROM communities WHERE id = $1")
                .bind(community.as_uuid())
                .fetch_one(&db.pool)
                .await
                .expect("community archive state");
