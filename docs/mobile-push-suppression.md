...
29. Preference changes do not support offline queuing. Show a pending state for up to approximately ten seconds while attempting the change. If it times out, treat the attempt as failed in the UI and revert to the previously confirmed value. Do not retain an offline change for later submission. A timeout does not prove the relay rejected the change. On the next successful sync, show the relay's actual setting, even if it differs from the value restored after timeout because the relay accepted the write but its acknowledgment was lost.
