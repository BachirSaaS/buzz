...
The operator can opt out per-agent with `BUZZ_GIT_IDENTITY=user`. In `user` mode the runtime installs no attribution machinery — no managed `git`, no injected identity, no signing enforcement — and commits carry the operator's own repo/global/system git identity and signing config. Inherited `GIT_CONFIG_*` identity and signing keys (`user.name`, `user.email`, `user.signingkey`, `gpg.format`, `gpg.x509.program`, `commit.gpgSign`, `tag.gpgSign`) are dropped. In both modes the spawned environment also loses `GIT_AUTHOR_NAME`, `GIT_AUTHOR_EMAIL`, `GIT_COMMITTER_NAME`, `GIT_COMMITTER_EMAIL`, `GIT_CONFIG_PARAMETERS` and `NOSTR_PRIVATE_KEY`. Relay git authentication works in both modes.
