...
test("bubbles have equal corners, unwrapped media, and aligned reply indicators", async ({
  ...
  for (const type of ["image", "video"]) {
    ...
    const caption = body
      ...
      .filter({ hasText: `A ${type} caption` });
    await expect(caption).toHaveCSS("border-radius", "24px");
  ...
  for (const corner of ["top-left", "top-right", "bottom-left", "bottom-right"])
    await expect(body).toHaveCSS(`border-${corner}-radius`, "24px");
  const bodyBox = await body.boundingBox();
...
test("bubbles follow the theme and anchor reactions and hover controls without row fill", async ({
  ...
  expect(await themeColor(page, "--primary")).toBe(
    await themeColor(page, "--blockui-fill-primary"),
  );
  await expect(page.getByTestId("unified-pulse")).toHaveCSS(
    "background-color",
    "rgba(0, 0, 0, 0)",
...
for (const appearance of ["light", "dark"] as const) {
  test(`Block UI ${appearance} conversation colors and typography scale together`, async ({
    page,
  }) => {
    ...
    await page
      ...
      .click();
    const detail = page.getByTestId("pulse-combined-detail");
    const incoming = detail.locator('[data-message-bubble="incoming"]').first();
    await expect(incoming).toBeVisible();
    await expect(incoming).toHaveCSS(
      "background-color",
      await themeColor(page, "--blockui-surface-standard"),
    );
    await expect(incoming).toHaveCSS(
      "color",
      await themeColor(page, "--blockui-text-standard"),
    );
    const before = await incoming.evaluate((node) => {
    ...
    expect(before.family).toContain("Inter");
    await page.evaluate(() => document.fonts.load('400 14px "Inter Local"'));
    expect(
      await page.evaluate(() => document.fonts.check('400 14px "Inter Local"')),
    ).toBe(true);
    const main = page.getByTestId("pulse-main-container");
    const dock = page.getByTestId("pulse-app-navigation");
    ...
    const channelIcon = page
      ...
      .first();
    await expect(channelIcon).toHaveCSS("width", "28px");
    await expect(channelIcon).toHaveCSS("height", "28px");
    await expect(channelIcon).toHaveCSS(
      "background-color",
      await themeColor(page, "--blockui-surface-standard"),
    );
    await waitForAnimations(page);
    await page.getByTestId("unified-pulse").screenshot({
      path: `test-results/pulse-prototype/blockui-${appearance}.png`,
    });
    await page.evaluate(() => {
    ...
    await expect
      .poll(() =>
        incoming.evaluate((node) =>
          parseFloat(getComputedStyle(node).fontSize),
        ),
      )
      .toBeGreaterThan(before.font);
    await expect
      .poll(() =>
        incoming.evaluate((node) =>
          parseFloat(getComputedStyle(node).borderRadius),
