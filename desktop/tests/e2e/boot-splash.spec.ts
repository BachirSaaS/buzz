...
// loading gate keeps the Block UI spinner up as an overlay above the already
...
test("boot splash overlay holds with a Block UI spinner, then dismisses", async ({
  page,
}) => {
  ...
  const spinner = overlay.locator('[data-slot="spinner"]');
