  ...
  const tabs = isOnboardingInline ? (
    ...
    >
      <TabsList
        ...
        className={cn(
          isOnboardingModal
            ? "relative isolate grid h-10 w-full max-w-[320px] grid-cols-3 overflow-hidden rounded-full bg-[color:color-mix(in_srgb,var(--buzz-onboarding-avatar-control-fg)_12%,transparent)] p-1 text-muted-foreground"
      ...
      >
        <div
          ...
          className={cn(
            ...
            isOnboardingModal
              ? "bg-[var(--buzz-onboarding-avatar-action-bg)]"
              : "bg-background",
          )}
          style={{
        ...
        {MODE_TAB_ORDER.map((tabMode) => (
          <TabsTrigger
            className={cn(
              isOnboardingModal
                ? "relative z-10 h-full rounded-full bg-transparent px-4 text-sm font-normal shadow-none transition-colors data-[state=active]:bg-transparent data-[state=active]:text-[var(--buzz-onboarding-avatar-action-fg)] data-[state=active]:shadow-none"
