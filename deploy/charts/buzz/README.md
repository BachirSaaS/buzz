...
Set `relay.nipFiMode=enforce` only for deployments that already require
client-attached NIP-FI evidence. The chart renders this as `BUZZ_NIP_FI_MODE`,
which makes NIP-11 advertise only the privacy-safe requirement shape from
`docs/nips/NIP-FI.md`: no issuer, audience, JWKS, Auth0/IdP, login, tenant, or
other identity details belong in chart values or discovery output. The default
`off` mode omits the discovery fields; `deny_protected` is a fail-closed repair
posture and intentionally does not advertise a usable client-auth contract.
