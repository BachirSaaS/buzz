...
try {
  await serveHost(directory,expected,async () => {
    ...
    return { close: () => running.close(), status: () => ({ revision: running.settingsRevision, agents: running.runningAgents, relay: transport.connected ? 'connected' as const : 'disconnected' as const }) };
  });
